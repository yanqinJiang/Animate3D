...
@dataclass
class HybridRandomCameraDataModuleConfig:
...
@dataclass
class HybridRandomCameraTestDataModuleConfig:
    # height, width, and batch_size should be Union[int, List[int]]
    # but OmegaConf does not support Union of containers
    height: Any = 64
    width: Any = 64
    ...
    light_position_perturb: float = 1.0
    light_distance_range: Tuple[float, float] = (0.8, 1.5)
    eval_elevation_deg: List[float]=field(default_factory=lambda: [15.0, 0., 30.])
    eval_azimuth_deg: List[List[float]]=field(default_factory=lambda: [[0., 90., 180., 270.], [30., 120., 210., 300.], [-45., 45., 135., 225.]])
    eval_camera_distance: float = 1.5
    eval_fovy_deg: float = 70.0
    light_sample_strategy: str = "dreamfusion"
    batch_uniform_azimuth: bool = True
...
class HybridRandomCameraIterableDataset(IterableDataset, Updateable):
    def __init__(self, cfg: Any) -> None:
        super().__init__()
        self.cfg: HybridRandomCameraDataModuleConfig = cfg
    ...
    def collate(self, batch) -> Dict[str, Any]:
        n_spatial_temporal_view = self.cfg.n_view * self.cfg.total_frame
        assert (
            self.batch_size % n_spatial_temporal_view == 0
        ), f"batch_size ({self.batch_size}) must be dividable by n_view ({n_spatial_temporal_view})!"
        real_batch_size = self.batch_size // n_spatial_temporal_view
        ...
        if random.random() < 0.5:
            ...
            ).repeat_interleave(n_spatial_temporal_view, dim=0)
        ...
        azimuth_deg = azimuth_deg.repeat_interleave(self.cfg.total_frame, dim=0)
        azimuth = azimuth_deg * math.pi / 180

        ######## Different from original, same as mvdream ########
        ...
        # time [-1, 1]
        total_timestamps = torch.linspace(-1, 1, steps=self.cfg.total_frame)
        time_indexs = torch.arange(0, self.cfg.total_frame).unsqueeze(0)
        time_indexs = time_indexs.unsqueeze(1).repeat(1, self.cfg.n_view, 1)

        timestamps = total_timestamps[time_indexs.reshape(-1)]
...
class HybridRandomCameraDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        ...
        self.timestamps = torch.linspace(-1, 1, steps=self.cfg.total_frame).unsqueeze(-1)
    ...
    def __getitem__(self, index):
        ...
        origin_index = index
        time_index = origin_index % self.cfg.total_frame
        # TODO: check this
        index = int(origin_index // self.cfg.total_frame) * self.cfg.total_frame
        ...
        return {
            ...
            "timestamps": self.timestamps[time_index]
        }
...
class HybridRandomCameraTestDataset(Dataset):
    def __init__(self, cfg: Any, split: str) -> None:
        super().__init__()
        self.cfg: HybridRandomCameraTestDataModuleConfig = cfg
        self.split = split
        
        if split == "val":
            self.n_views = self.cfg.n_val_views
        else:
            self.n_views = len(self.cfg.eval_elevation_deg) * len(self.cfg.eval_azimuth_deg[0])
        ...
        if self.split == "val":
            ...
            azimuth_deg = torch.tensor(self.cfg.eval_azimuth_deg).reshape(-1)
        
        elevation_deg = torch.tensor(self.cfg.eval_elevation_deg).repeat_interleave(len(self.cfg.eval_azimuth_deg[0]))
        ...
        self.timestamps = torch.linspace(-1, 1, steps=self.cfg.total_frame).unsqueeze(-1)
        # for 4dfy
        # num_frames = self.cfg.total_frame
        # t0 = 1 / self.cfg.total_frame * 0.5
        # frame_times = torch.linspace(
        #     t0, t0 + (num_frames - 1) / num_frames, num_frames
        # )
        # self.timestamps = 2 * frame_times - 1


    def __len__(self):
        return len(self.cfg.eval_azimuth_deg[0]) * len(self.cfg.eval_elevation_deg) * self.cfg.total_frame
    ...
    def __getitem__(self, index):
        ...
        time_index = origin_index % self.cfg.total_frame
        # TODO: check this
        index = int(origin_index // self.cfg.total_frame)
...
class HybridRandomCameraDataModule(pl.LightningDataModule):
    cfg: HybridRandomCameraDataModuleConfig

    def __init__(self, cfg: Optional[Union[dict, DictConfig]] = None) -> None:
        super().__init__()
        self.cfg = parse_structured(HybridRandomCameraDataModuleConfig, cfg)

    def setup(self, stage=None) -> None:
        if stage in [None, "fit"]:
            self.train_dataset = HybridRandomCameraIterableDataset(self.cfg)
        if stage in [None, "fit", "validate"]:
            self.val_dataset = HybridRandomCameraDataset(self.cfg, "val")
        if stage in [None, "test", "predict"]:
            self.test_dataset = HybridRandomCameraDataset(self.cfg, "test")
